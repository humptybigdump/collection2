...
!----- 3: refinement
!-----    minn : Newton-Raphson
!-----    dire : direction of endmembers
!-----    dirs : direction of structural endmembers
!
      SUBROUTINE CALSTR
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
...
!******************************
      ...
      include 'files.cmn'
!-----END OF COMMON VARIABLES
      INTEGER*4 I,II,IIX,I001,K,LOO2,IS,CLCOD,ADDCOD,J
      REAL*8 SUMME,XXSC(EMAX),GSC,NEWN(COMAX)
      CHARACTER*20 CH23
      CHARACTER*250 ASE1,ASE2
      LOGICAL*4 CODE,AFAIL,SAMEAS
!-----
      NMAX=NPHA
      NMAXMAX=NPHA
      SUGLOOP=NPHA+1
...
!******************************
      ...
      include 'files.cmn'
!-----END OF COMMON VARIABLES
      INTEGER*4 IS,I1,START,I001,I002,I,I2,J,II,ADDCOD
...
!C      DO 420,I=1,NUN
!C      IF (NUMMER(I).GT.0) THEN
!C      IF (EMSOL(NUMMER(I)).GT.0) THEN
!CC==
!C      IF (TEST.LT.0.0D0) THEN
!C      WRITE (UNIT=6,FMT='('' add close to stem'')')
!C      WRITE (UNIT=out,FMT='('' add close to stem'')')
!C      END IF
!CC==
!C      IS=EMSOL(NUMMER(I))
!C      DO 422,J=1,NEND(IS)
!C  422 XXEM(J)=1D-10
!C      XXEM(EMNR(NUMMER(I)))=1.0D0-DBLE(NEND(IS)-1)*1D-10
!!add SUGCOD!
!C      CALL NEWPH(IS)
!C      END IF
!C      END IF
...
!CCCCC
!2      IF (NNEGEM(IS).GT.0.AND.I1.GT.0) I1=0
      I1=-NEND(IS)
      I2=LOCMIN(IS)
...
!      IF (I1.GT.0) I1=0
!      IF (I2.LT.0) I2=0      
!cdcFeb11 for the moment: do not start with em, but only scan
...
!******************************
      ...
      include 'files.cmn'
!-----END OF COMMON VARIABLES
      INTEGER*4 IS,I,CODE,N
      REAL*8 XXSC(EMAX),GSC,FF
!-----
...
!==
      ...
      CALL GNONID(IS,XXSC,GSC)
      GSOL=GSC
!+++++ above is added for security (last replaces calls before NEWPH)
!DC
      NMAX=NMAX+1
      SUGNR=SUGNR+1
      IF (NMAX.GT.CALMAX) THEN
...
!-----END OF COMMON VARIABLES
      ...
      F001,AR001(EMAX),FN,FX,FF,XXSC(EMAX)
!-----
      IF (TEST.LT.0.0D0) THEN
...
!-----
!******************************
      SUBROUTINE MARMIN(IS,START,XXSC)
...
!-----END OF COMMON VARIABLES
      ...
      I1,I2,I3,I4,FRAC,COMBO(10),IC
      REAL*8 XXSC(EMAX),GSC,XXX(3,EMAX),GGG(3),AR001(EMAX), &
      AR002(EMAX),SUMME,F001,FN,FF,FX,LESCAN,MUE(EMAX)
      CHARACTER*133 CH001
      CHARACTER*6 MINKOM
!==
      IF (TEST.LT.0.0D0) THEN
      WRITE (UNIT=6,FMT='('' START = '',I4,2X,A16)') START,SOLNAM(IS)
      WRITE (UNIT=6,FMT='('' ------------'')')
      WRITE (UNIT=out,FMT='('' START = '',I4,2X,A16)') START,SOLNAM(IS)
      WRITE (UNIT=out,FMT='('' ------------'')')
      END IF
!-----INITS
...
!-----step 1 and 2
      L1=1
      IF (TRY.GT.1.AND.GTOT.LE.GTEST) NSTEP=500
...
!-----step 3 to n
  ...
  525 CONTINUE
!=====
!-----direction of endmembers
   15 CALL DISTAN(IS,AR001,XXEM)
      STEP2=0
  ...
  530 CONTINUE
!=====
!-----direction of structural endmembers
...
!==
  ...
  532 WRITE (UNIT=CH001(I002:),FMT='(F10.6)') XXEM(I)
      ...
      WRITE (UNIT=out,FMT='(A133)') CH001
  533 CH001=' '
      WRITE (UNIT=scr,FMT=106) DELTAX,DISTA,GNOM
      WRITE (UNIT=out,FMT=106) DELTAX,DISTA,GNOM
...
!----- use as starting point for MINN
      DO 310,I=1,NEND(IS)
  310 XXSC(I)=XXEM(I)
      CALL MINN(IS,XXSC,GSC)
!+++++
!=====add more to pool
!-----end-start *(1 +/- F001)
!C      IF (DISTA.NE.0.0D0) THEN
!C      IF (TEST.LT.0.0D0) THEN
!C      WRITE (UNIT=scr,FMT='(1X,''end-start *(1 +/- F001):'')')
!C      WRITE (UNIT=out,FMT='(1X,''end-start *(1 +/- F001):'')')
!C      END IF
!C      DO 540,II=-1,1,2
!CCdC   F001=DBLE(II)*10.0D0*DXMIN
!CCdC   F001=DBLE(II)*0.5D0
!C      F001=DBLE(II)*0.1D0
!C      DXEND=DMAX1((1.0D0+F001)*DISTA,DXMIN)
!C      CALL VECADD(IS,AR001,F001,XXSC)
!C      CALL GNONID(IS,XXSC,GSC)
!C      DO 541,I=1,NEND(IS)
!C  541 XXEM(I)=XXSC(I)
!C      GSOL=GSC
!!add SUGCOD!
!C      CALL NEWPH(IS)
...
!CDC      DXEND=DMAX1(DISTA,DXMIN)
!      DXEND=DMAX1(DISTA,0.01D0)
!      FN=DBLE(NEND(IS))
!      FX=DSQRT((FN-1.0D0)/FN)
!      FF=-DSQRT(1.0D0/(FN*(FN-1.0D0)))
!      DO 650,I=1,NEND(IS)
!  650 VEKTOR(I)=FF
!      F001=DMAX1(DXEND/2.0D0,DXMIN)
!      DO 700,II=1,NEND(IS)
!      IF (II.NE.1) VEKTOR(II-1)=FF
!      VEKTOR(II)=FX
!      CALL VECADD(IS,AR001,F001,XXSC)
...
!-----direction of endmembers
      IF (NEND(IS).LE.2) RETURN
!DC      DXEND=DMAX1(DISTA,DXMIN)
      IF (TEST.LT.0.0D0) THEN
      WRITE (UNIT=scr,FMT='(1X,''direction of endmembers:'')')
      WRITE (UNIT=out,FMT='(1X,''direction of endmembers:'')')
      END IF
      DXEND=DMAX1(DISTA,0.1D0)
      DO 650,I=1,NEND(IS)
  650 VEKTOR(I)=-AR001(I)
      DO 700,II=1,NEND(IS)
      IF (II.NE.1) VEKTOR(II-1)=AR001(II-1)
      VEKTOR(II)=1.0D0-AR001(II)
...
!===
      IF (NSTREM(IS).GT.0) THEN
      IF (TEST.LT.0.0D0) THEN
      WRITE (UNIT=scr,FMT='(1X,''direction of str. endmembers:'')')
      WRITE (UNIT=out,FMT='(1X,''direction of str. endmembers:'')')
      END IF
      DO 740,II=1,NSTREM(IS)
      DXEND=DMAX1(DISTA,0.1D0)
      DO 750,I=1,NEND(IS)
  750 VEKTOR(I)=STREM(IS,II,I)-AR001(I)
      CALL GNONID(IS,AR001,GSC)
      DELTAX=1D-6
      CALL ETC(IS,AR001,GSC,XXSC,F001)
!cdcFeb2011
...
!-----
!******************************
      SUBROUTINE MINN(IS,XX0,GG0)
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
      include 'files.cmn'
!-----END OF COMMON VARIABLES
      INTEGER*4 IS,I,II,N0,IE,IMEGA,NRLOOP,ERRC,N1,CODE,BCOD, &
      PINDEX(COMAX),MINDEX(COMAX),IP,IM,I0
      ...
      MINXV(COMAX),XX0N(EMAX),GLAST,GPROG,GPROGTOT,RTEST
!-----
      ERRC=0
      RTEST=0.0D0
      N0=NEND(IS)
...
!----- (caused at some times problems e.g. with fo_qz_melt) ???
      ...
      include 'files.cmn'
!-----END OF COMMON VARIABLES
      INTEGER*4 II,IIX,IIY,I001,PIV,IC,IR
