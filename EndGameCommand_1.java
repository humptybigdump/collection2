...
import codefight.model.ai.State;
...
final class EndGameCommand implements Command {
    ...
    private static final String ERROR_NO_ARGUMENTS = "this command does not accept any arguments";

    private static final String HELP_MESSAGE = "Ends the game";
    private final Phase phase = Phase.GAME_PHASE;
    ...
    public CommandResult execute(CodeFight model, String[] commandArguments) {
        ...
        if (phase != model.getPhase()) {
            ...
            return new CommandResult(CommandResultType.FAILURE, ERROR_NO_ARGUMENTS);
        }
        ...
        for (Player participant : participants) {
            if (participant.getState() == State.RUNNING) {
                running.append(participant);
                running.append(LIST_DIVIDER);
            } else if (participant.getState() == State.STOPPED) {
                stopped.append(participant);
                stopped.append(LIST_DIVIDER);
            }
        }
