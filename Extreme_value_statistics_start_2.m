...
c=0;
% compute plotting positions and return periods
PP_emp=;
RP_emp=;
...
% Compute theoretic Probabilities and return periods
PP_theo=;
RP_theo=;
...
subplot(2,1,1);
plot(Q_sort,PP_emp,'ro-','linewidth',2);
hold on;
plot(Q_sort,PP_theo,'b+-','linewidth',2);
xlabel(' HQ [m^3/s]','fontsize',16);
ylabel('Empirical probability ','fontsize',16);
set(gca,'fontsize',16,'linewidth',2);
ylim([0 1.1]);
subplot(2,1,2);
plot(Q_sort,RP_emp,'ro-','linewidth',2);
hold on;
semilogy(Q_sort,RP_theo,'b+-','linewidth',2);
xlabel(' HQ [m^3/s]','fontsize',16);
...
% Define bining for histogramms
Class_low=[50:50:500]'; % Upper limit
Class_high=[100:50:550]';% lower limit
% Classes for empirical histogramm
subplot(2,1,2);
Class_emp=[75:50:525];% class centers
...
Chi_square_theo=chi2inv(0.01,7);
