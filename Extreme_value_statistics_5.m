...
% Fit Gumble distribution using the method of momens
close all;
clear all;
% Read input discharge time series/designflood 
data= readtable('Iller_Extreme_no_header.csv','Delimiter', ';');

% define the runoff column as vector
time= table2array(data(:,{'Year'})); % time
Q_peak= table2array(data(:,{'Q'})); % Peak discharge
Rank= table2array(data(:,{'Rank'})); % rank
Q_sort= table2array(data(:,{'Q_sort'})); % HQ rank sorted

[nline ncol]=size(data); % size of the data matrix
...
% Plotting position, note that c is zero for floods 
c=0;
% compute plotting positions/non exceedence probabilities and return using ranks as in lecture
PP_emp=(Rank-c)/(nline+1-2*c);
RP_emp=1./(1-PP_emp);% return period

%Compute parameter of gumbel distribution
HQ_ave=mean(Q_sort); % mean flood discharge
HQ_std=std(Q_sort);  % standard deviation of flood discharges
% HQ_ave=100;
% HQ_std=5;
Gumbel_a=pi()/(HQ_std*sqrt(6));
Gumbel_b=HQ_ave-0.5772/Gumbel_a;
% Calculated theoretical non exceedent probalities using the Gumpel
% Parameters and sorted HQ
PP_theo=exp(-exp(-Gumbel_a*(Q_sort-Gumbel_b*ones(nline,1))));
RP_theo=1./(1-PP_theo); % theoretical return period

% Plot sample and fitted CDF in probability space and return period space
figure;
...
% Define upper and lower boundaries of classes (bin), bin widht 50 m3/s
Class_low=[50:50:500]'; % Lower boundaries
Class_high=[100:50:550]'; % Lower boundaries
% Middle points of the bins for histogramm plottin
Class_emp=[75:50:525];
% Compute theoretical histogram
% non exceedence probability lower class limits hist1_theo
hist1_theo=exp(-exp(-Gumbel_a*(Class_low-Gumbel_b*ones(length(Class_low),1))));
% non exceedence probability upper class limits hist2_theo
hist2_theo=exp(-exp(-Gumbel_a*(Class_high-Gumbel_b*ones(length(Class_low),1))));
% theoretical historgam (hist2_theo -hist1_theo)*N
...
% Calculate Chi-square values according to formula in the lecture notes
