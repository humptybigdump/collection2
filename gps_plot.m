...
%------------------------------------------------------------------
    ...
    %   Diese Originalzeile verursacht eine Fehlermeldung in quiver
    %   Nach einigen Probieren folgende Loesung gefunden
    %   MW, 07.02.2011
    %-----------------------------------------------------        
        blax=[aaa(1,1),aaa(1,1)];
        blex=[bbb(1,2),bbb(1,2)]';
        blux=[unit,unit;unit,unit]*LON_PER_X;
        a0 = quiver((blax+xinc),(blex+yinc*1.5),blux*resz,blux*0.0,0); %scale vector (1m)
    %----------------------------------------------------
...
%         h1 = text((aaa(1,1)+2.0*xinc),(bbb(1,2)+yinc*3.0),'1m');
    ...
    %   MW, 07.02.2011
    %-----------------------------------------------------        
        blax=[XGRID(1),XGRID(1)];
        blex=[YGRID(1),YGRID(1)]';
        blux=[unit,unit;unit,unit];
        a0 = quiver((blax+xinc),(blex+yinc*1.5),blux*resz,blux*0.0,0); %scale vector (1m)
    %----------------------------------------------------
