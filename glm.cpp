 ...
 *
 *  modifications, e.g. link to VAO/VBO wrapper, C. Dachsbacher
 */
#include "stdafx.h"
...
/* _glmMin: returns the minimum of two floats */
static GLfloat
...
_glmFirstPass(GLMmodel* model, FILE* file) 
{
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case 'u':
      /* eat up rest of line */
      fgets(buf, sizeof(buf), file);
	  sprintf(mbuf, "dummygroup_%d", dummyGroupIdx ); dummyGroupIdx ++;
      group = _glmAddGroup(model, mbuf);
      break;
    case 'g':				/* group */
      /* eat up rest of line */
      fgets(buf, sizeof(buf), file);
...
GLuint    numvertices;		/* number of vertices in model */
  ...
  material = _glmFindMaterial(model, (char*)"default" );
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case 'u':
	  ...
	  sprintf(mbuf, "dummygroup_%d", dummyGroupIdx ); dummyGroupIdx ++;
      group = _glmFindGroup(model, mbuf);
...
glmUnitize(GLMmodel* model)
{
  ...
  maxx = maxy = maxz = -1e37f;
  minx = miny = minz = 1e37f;
  GLMmodel *obj = model;
	for ( GLMgroup *group = obj->groups; group; group = group->next )
	if ( group->numtriangles > 0 )
	if ( !( obj->materials[ group->material ].diffuse[ 0 ]==1.0f && obj->materials[ group->material ].diffuse[ 1 ]==0.0f && obj->materials[ group->material ].diffuse[ 2 ] == 0.0f ) )
	{
		float x, y, z;
		for ( int i = 0; i < (int)group->numtriangles; i++ )
		for ( int k = 0; k < 3; k++ )
		{
			x = obj->vertices[ (T(group->triangles[i]).vindices[ k ] * 3) + 0 ];
			y = obj->vertices[ (T(group->triangles[i]).vindices[ k ] * 3) + 1 ];
			z = obj->vertices[ (T(group->triangles[i]).vindices[ k ] * 3) + 2 ];
			maxx = _glmMax( x, maxx );
			maxy = _glmMax( y, maxy );
			maxz = _glmMax( z, maxz );
			minx = _glmMin( x, minx );
 ...
 */
GLvoid
glmWriteOBJ(GLMmodel* model, char* filename, GLuint mode, GLMgroup *onlyThisGroup )
{
  GLuint    i;
  FILE*     file;
  GLMgroup* group;
  ...
  group = model->groups;
  while(group) {
    if ( group == onlyThisGroup || group == NULL )
	{
		fprintf(file, "g %s\n", group->name);
...
GLvoid
glmDraw2Wrapper(GLMmodel* model, GLuint mode, IMWrap *wrap )
{
  GLuint i;
  GLMgroup* group;
  ...
  while (group) {
    if (mode & GLM_COLOR) {
		wrap->Attrib3fv( idxColor, model->materials[group->material].diffuse);
    }

    for (i = 0; i < group->numtriangles; i++) {
      if (mode & GLM_FLAT)
		wrap->Attrib3fv( idxNrml, &model->facetnorms[3 * T(group->triangles[i]).findex]);
      
      if (mode & GLM_SMOOTH)
		wrap->Attrib3fv( idxNrml, &model->normals[3 * T(group->triangles[i]).nindices[0]]);
      if (mode & GLM_TEXTURE)
		wrap->Attrib2fv( idxTexCoord, &model->texcoords[2*T(group->triangles[i]).tindices[0]]);
      wrap->Attrib3fv( 0, &model->vertices[3 * T(group->triangles[i]).vindices[0]]);
	  wrap->emitVertex();
      
      if (mode & GLM_SMOOTH)
		wrap->Attrib3fv( idxNrml, &model->normals[3 * T(group->triangles[i]).nindices[1]]);
      if (mode & GLM_TEXTURE)
		wrap->Attrib3fv( idxTexCoord, &model->texcoords[2*T(group->triangles[i]).tindices[1]]);
      wrap->Attrib3fv( 0, &model->vertices[3 * T(group->triangles[i]).vindices[1]]);
	  wrap->emitVertex();
      
      if (mode & GLM_SMOOTH)
		wrap->Attrib3fv( idxNrml, &model->normals[3 * T(group->triangles[i]).nindices[2]]);
      if (mode & GLM_TEXTURE)
		wrap->Attrib3fv( idxTexCoord, &model->texcoords[2*T(group->triangles[i]).tindices[2]]);
      wrap->Attrib3fv( 0, &model->vertices[3 * T(group->triangles[i]).vindices[2]]);
	  wrap->emitVertex();
