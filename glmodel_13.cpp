...
# include <random>
# include <cglib/core/spherical_harmonics.h>
	...
	: flags(_flags)
{
	...
	bool prt = flags & PRT;
	if(prt) {
		std::vector<std::array<float, NUM_COEFFICIENTS>> sh_coefficients;

		compute_prt(path, vertices, normals, sh_coefficients);

		glGenBuffers(1, &buf_prt);
		glBindBuffer(GL_ARRAY_BUFFER, buf_prt);
		glBufferData(GL_ARRAY_BUFFER, sizeof(sh_coefficients[0]) * sh_coefficients.size(), sh_coefficients.data(), GL_STATIC_DRAW);
		glEnableVertexAttribArray(3);
		glEnableVertexAttribArray(4);
		glEnableVertexAttribArray(5);
		glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(float) * NUM_COEFFICIENTS, (const char *) 0 +  0 * 3 * sizeof(float));
		glVertexAttribPointer(4, 3, GL_FLOAT, GL_FALSE, sizeof(float) * NUM_COEFFICIENTS, (const char *) 0 +  1 * 3 * sizeof(float));
		glVertexAttribPointer(5, 3, GL_FLOAT, GL_FALSE, sizeof(float) * NUM_COEFFICIENTS, (const char *) 0 +  2 * 3 * sizeof(float));
	}
