% Macro compare random field and Turning band generated field or Kriging interpolated field 
...
data=load('ks_gaus_0,5_1,5_100.dat'); % structure x y z
dataip=load('Interpolation_ks_gauss_400.dat'); % structure x y z
%
var1=var(data(:,3));
varip=var(dataip(:,3));
rand('seed',1);
...
set(gca,'fontsize',16,'linewidth',2);
title('log(ks)','fontsize',16);
titel=sprintf('Interpolation. %f5.2',varip);
title(titel,'fontsize',16);
shading flat;
h=colorbar;
caxis([-8 -1]);
...
plot(xm,ym,'k+');
colormap jet;
%   Compile univariate statistics
figure;
subplot(3,1,1);
[x y]=hist(dataip(:,3));
bar(y, x/length(dataip(:,3)));
titel= ['Ordinary kriging mean & var' num2str(mean(dataip(:,3))),', ' num2str(std(dataip(:,3))^2)];
title(titel,'fontsize',16);
ylim([0 0.7]);
xlim([-12 0])
set(gca,'fontsize',16,'linewidth',2);
