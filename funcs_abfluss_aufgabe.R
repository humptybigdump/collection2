...
plofun_event_attert <- function( loesung = TRUE)
{
  ...
  # 1 # Regen
   ...
   par(mar= c(schmal, normal, schmal, rechts)+0.1)
    ...
    # 2 # Weierbach
     plot(event$Weierbach, xaxt="n", bty="l",  ylab="Q /(m³/s)",
           ylim=c(0,2.5e-2))
      if(loesung)
    {  abline(v=c(rain_start, rain_end), lty=c(2,1), lwd=2,col="orange")
      ...
      text(x = end.wei+3600, adj=0,y=0.75*par("usr")[4],
           labels=bquote(V[direkt] == .(est.vol.wei) ~ "m³") ,
           col=4)
      }
    ...
    # 3  Colpach
      plot(event$Colpach, xaxt="n",  ylab="Q /(m³/s)",
           ylim=c(0,1.6)
           , yaxp=c(0,1.5, 5)
           )
       ...
       { abline(v=c(rain_start, rain_end), lty=c(2,1), lwd=2,col="orange")
         ...
         text(x = end.col+3600, adj=0,y=0.75*par("usr")[4],
               labels=bquote(V[direkt] == .(est.vol.col) ~ "m³") ,
               col=4)
    ...
    par(mar= c(normal, normal, schmal, rechts)+0.1)
      plot(event$Useldange, xaxt="n",  ylab="Q /(m³/s)", xlab="",
            ylim=c(0,26))
       ...
       { # Linie regenereignis
        ...
        # Polygon
         ...
         labels=bquote(V[direkt] == .(est.vol.usl) ~ "m³") ,
         col=4)
         }
