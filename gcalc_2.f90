...
!-----END OF COMMON VARIABLES
      ...
      K0,K01,K02,NN0,DH,DV,PK0,DXP,PXP,PXM,F1,F2, &
      PJ,GCH,HCH,SCH,VCH,CPCH,DG1,DG2,SII
      INTEGER*4 I,I001,IP
!=====
      FGVOL=0.0D0
      FVVOL=0.0D0
...
!---- check for possible database errors
...
!===== it is almost the same as 1998, but DKDT always 1.5D-4
...
!---- dkdt ist nach hopo 1998 immer -1.5D-4
      A22=10.0D0
!---- KAPS is D2 and equations adapted
!      KAPS=4.0D0
      KAPS=D2
      K0=D1
      DKDT=VL0
      ...
      FV=V0R*(1.0D0+A00*(T-T0)-20.0D0*A00*(SQT-SQT0))
      VOLUM=FV*(KTT/(KAPS*PK+KTT))**(1.0D0/KAPS)
      FVVOL=VOLUM-V0R
      FGVOL=1D3*(FV*KTT/(KAPS-1.0D0))* &
      (((KAPS*PK+KTT)/KTT)**((KAPS-1.0D0)/KAPS)- &
      ((KAPS*PK0+KTT)/KTT)**((KAPS-1.0D0)/KAPS))
      GR=GR+FGVOL
      END IF
!-----
...
!-----END OF COMMON VARIABLES
      REAL*8 AA,BB,DGGAS
      COMMON /GORE/ AA,BB,DGGAS
      REAL*8 VREF,VOL,VGAS,VFL,FVHALF,THRHALF,TWOTHRD,RKJ,PREF
...
!-----END OF COMMON VARIABLES
      ...
      REAL*8 KUB,KUC,KUD,OFT,VREF,VOL,VGAS,VFL,X1,X2,X2I,X3,F001
!
      REAL*8 FVHALF,THRHALF,TWOTHRD,PREF,RKJ
      REAL*8 CSA0,CSA1,CSB0,CSC0,CSC1,CSD0,CSD1
