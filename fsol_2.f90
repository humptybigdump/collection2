...
!********************************
      ...
      A1,A2,A3,A4,A5,LNG1,LNG2,LNG3,LNG4,LNG5,FF,YY,NN,XX,ZZ,AA, &
      ...
      FE,AL,TI,MG,VAT,AC1,AC2,GTOT,SAR(15),F1PLO,F2PLO, &
...
! following **5 according to white et al 2007, why?????
      A(6)=(1.0D0-X(8))*X(6)*F1**5
      A(7)=(1.0D0-X(8))*X(7)*F2**5
      A(8)=X(8)**2
      END IF
...
!----
!-----
      REAL*8 TC,T,P,XF(2),VAT,AC1,AC2,VOLUM, &
...
!--
      REAL*8 T,P,V,Z, &
      VOUT,V2,V4,V5,DELTA,POUT,START,XX,INTP(5),VOLP(5),FF, &
      VOLST,INTST,ZP(5),EQ11,EQ10,ZST
...
!--------------------------------
      ...
      IMPLICIT NONE
      REAL*8 T,P,V,Z,PHI1,PHI2, &
      VOUT,V2,V4,V5,DELTA,POUT,EXPON
!--
      REAL*8 BVC,CVC2,DVC4,EVC5,FVC2,BETA1,GAMMAVC2,R,RT,MWT, &
      BST1,CST1,DST1,EST1,FST1,BESTST1,GAMST1, &
      BST2,CST2,DST2,EST2,FST2,BESTST2,GAMST2
...
!--
      REAL*8 T,P,V,Z, &
      VOUT,V2,V4,V5,START,DELTA,POUT,P1,P2,P3,X1,X2,X3, &
      VMAXI,PMINI,PDEL,DX,XX,XDEL,VXMINI,PST,AB1,AB2,AB3,VVOR
...
!--------------------------------
      ...
      IMPLICIT NONE
!-----
      REAL*8 T,K1(2,2),K2(2,2,2),K3(2,2,2),T2,FF
      INTEGER*4 I,J,K,N
!-----
!--- bis jetzt nur index 1 und 2
!---- 1=H2O, 2=CO2, 3=CH4
...
!      
      IMPLICIT NONE
      REAL*4 X(12),Y(12),Y2(12),YP1,YPN,XX,YY,T
      INTEGER*4 N,KN
      N=12
      YP1=0.0D0
      YPN=0.0D0
...
!-- critical density H2O: 0.017873993893977 mol/ccm
      ...
      COMMON /PISTRE/ CC,RT
!--
      REAL*8 TC,T,P,RH,R,ZZ,MI,MA,LIM1,LIM2,RHINI, &
      VOL1,VOL2,VOL0,VDP,VDP1,VDP2,PDV,F1,RH0,RH1,RH2,VRE,GRE
      INTEGER*4 I,I0,I1,I2,IM
!--
...
!--
      REAL*8 CC(10),RT
      COMMON /PISTRE/ CC,RT
      REAL*8 Z,VMIN,VMAX,G,DX,DXMIN,XU,XO,YU,YO, &
      X(100),Y(100),YPREC,F1,XD
      INTEGER*4 I,I1,I2,IMEGA,II,IC,IM
...
!--------------------------------
!--------------------------------
      SUBROUTINE INTEGP(VMIN,VMAX,G)
      IMPLICIT NONE
!--
      REAL*8 CC(10),RT
      COMMON /PISTRE/ CC,RT
      REAL*8 Z,VMIN,VMAX,G,V1,V2,DX,P1,P2
      INTEGER*4 I
!-----
      G=0.0D0
      DX=(VMAX-VMIN)/100000.0D0
      V1=VMIN
      CALL PISTPV(V1,P1)
      DO I=1,100000
       V2=V1+DX
...
!--
      REAL*8 CC(10),RT
      COMMON /PISTRE/ CC,RT
      REAL*8 Z,RH,RH2,RH3,RH4,F1,F2,ZS
!----- mit maxima
...
!--------------------------------
!--------------------------------
      SUBROUTINE PISTZSF(RH,ZS)
      IMPLICIT NONE
!--
      REAL*8 CC(10),RT
      COMMON /PISTRE/ CC,RT
      REAL*8 RH,ZS,RH1,RH2,Z1,Z2,DX
!----- mit finiten diff
      DX=RH/1D4
      RH1=RH
      CALL PISTZ(RH1,Z1)
...
!--------------------------------
!--------------------------------
      SUBROUTINE PISTZS2(RH,ZS2)
      IMPLICIT NONE
!--
      REAL*8 CC(10),RT
      COMMON /PISTRE/ CC,RT
      REAL*8 RH,ZS2,RH1,RH2,Z1,Z2,DX
!----- mit finiten diff
      DX=RH/1D4
      RH1=RH
      CALL PISTZS(RH1,Z1)
